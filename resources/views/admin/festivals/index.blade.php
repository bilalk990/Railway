            ...
            <div class="d-flex align-items-center flex-wrap mr-1">
                <div class="d-flex align-items-baseline flex-wrap mr-5">
                    <h5 class="text-dark font-weight-bold my-1 mr-5">
                       {{Config('constants.FESTIVAL.FESTIVAL_TITLES')}}
                   </h5>
                ...
                <div class="card card-custom card-stretch card-shadowless">
                    <div class="card-header">
                        ...
                        <div class="card-toolbar">
                            ...
                            <a href='{{route("$model.create")}}' class="btn btn-primary"> {{ trans("Add New ") }}{{Config('constants.FESTIVAL.FESTIVAL_TITLE')}}</a>
                        </div>
                    </div>
                    <div class="card-body">
        ...
        <select name="state_id" class="form-control select2" style="width: 100%;">
            ...
            @if(!empty($allStates))
                @foreach($allStates as $state)
                    <option value="{{ $state->id }}" {{ (isset($searchVariable['state_id']) && $searchVariable['state_id'] == $state->id) ? 'selected' : '' }}>
                        {{ $state->name }}
                    </option>
                @endforeach
            @endif
                                        ...
                                        {{-- <div class="col-lg-2 mb-lg-5 mb-6">
                                            ...
                                            <select name="is_popular" class="form-control">
                                                ...
                                                <option value="1" {{ (isset($searchVariable['is_popular']) && $searchVariable['is_popular'] == '1') ? 'selected' : '' }}>Popular</option>
                                                <option value="0" {{ (isset($searchVariable['is_popular']) && $searchVariable['is_popular'] == '0') ? 'selected' : '' }}>Not Popular</option>
                                            </select>
                                        </div> --}}
                ...
                @else
                    <span class="text-muted">No states assigned</span>
                @endif
            </td>
        ...
        <div class="toggle-button">
            <label class="switch mb-0">
                <input 
                    ...
                    data-id="{{ $result->id }}" 
                    data-popular="{{ $result->is_popular }}"
                    {{ $result->is_popular ? 'checked' : '' }}
                    id="popular_{{ $result->id }}"
                >
                <span class="slider round"></span>
            </label>
        </div>
        <span class="ml-2 status-text {{ $result->is_popular ? 'text-success' : 'text-danger' }}">
            {{ $result->is_popular ? 'Popular' : 'Not Popular' }}
        </span>
    </div>
</td> --}}
                                            ...
                                            <div class="dropdown dropdown-inline">
                                                ...
                                                <div class="dropdown-menu dropdown-menu-sm dropdown-menu-right">
                                                    <ul class="nav nav-hoverable flex-column">
                                                        <li class="nav-item">
                                                            <a class="nav-link" href="{{route('festivals.festivalTemples',array(base64_encode($result->id)))}}">
                                                                <span class="nav-text">Temples</span>
                                                            </a>
                                                        </li>
                                                         <li class="nav-item">
                                                                <a class="nav-link" href="{{ route('faqs.festivalIndex', ['id' => base64_encode($result->id)]) }}">
                                    ...
                                    @else
                                    <tr>
                                        <td colspan="7" style="text-align:center;"> {{ trans("Record not found.") }}</td>
                                    </tr>
                                    @endif
                                </tbody>
                            </table>
...
$(document).ready(function() {
    // Handle popular/unpopular toggle
    $('.popular-toggle').on('change', function() {
        var festivalId = $(this).data('id');
        var isPopular = $(this).is(':checked') ? 1 : 0;
        var switchElement = $(this).parent('.switch');
        var statusText = $(this).closest('td').find('.status-text');
        
        // Show loading state
        switchElement.addClass('loading');
        $(this).prop('disabled', true);
        
        // Send AJAX request
        $.ajax({
            url: '{{ route("festivals.markPopular") }}',
            type: 'POST',
            data: {
                _token: '{{ csrf_token() }}',
            ...
            success: function(response) {
                if (response.success) {
                    // Update status text
                    statusText.text(isPopular ? 'Popular' : 'Not Popular');
                    statusText.removeClass('text-success text-danger');
                    statusText.addClass(isPopular ? 'text-success' : 'text-danger');
            ...
            complete: function() {
                ...
                switchElement.removeClass('loading');
                $(this).prop('disabled', false);
            }.bind(this) // Bind 'this' to the complete function
        });
    });
    
    // Prevent double-click on toggle
    $('.popular-toggle').on('click', function(e) {
